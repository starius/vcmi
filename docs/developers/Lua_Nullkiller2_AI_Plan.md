...
- Pure Lua tests, fixture-based differential smoke tests, and JSON replay fixtures run through
  ...
  fixed-answer query callbacks, `showMapObjectSelectDialog`, `showBlockingDialog`, and
...
map-object selection, blocking-dialog, and surrender/retreat decisions and will be extended decision by decision.
...
5. Expand the native `Nullkiller2` trace exporter beyond fixed-answer query, map-object selection, blocking-dialog,
   and surrender/retreat decisions so discrepancies produce minimized fixtures under
