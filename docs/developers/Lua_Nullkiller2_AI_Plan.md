...
- Pure Lua tests, fixture-based differential smoke tests, and JSON replay fixtures run through
  ...
  fixed-answer query callbacks, artifact/state/battle status callbacks, `heroGotLevel`, passive UI status
  callbacks, `showMapObjectSelectDialog`, `showBlockingDialog`, `showTeleportDialog`, and
...
visited-town recruitment helper with duplicate-stack merging and resource-capped purchases.
...
- `ExecuteHeroChain` replays path nodes in native backward order, executes the first set of Lua-owned special-action
  ...
  Dimension Door plans, and invokes Lua-owned siege formation for immediate enemy-town attacks, but live host
...
artifact/state/battle status, hero-level-up, passive UI status, map-object selection, blocking-dialog,
teleport-dialog, and surrender/retreat decisions and will be extended decision by decision.
...
5. Expand the native `Nullkiller2` trace exporter beyond fixed-answer query, artifact/state/battle status,
   hero-level-up, passive UI status, map-object selection, blocking-dialog, teleport-dialog, and surrender/retreat
   decisions so discrepancies produce minimized fixtures under
