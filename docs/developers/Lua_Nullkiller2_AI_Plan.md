...
- Pure Lua tests, fixture-based differential smoke tests, and JSON replay fixtures run through
  ...
  fixed-answer query callbacks, `heroGotLevel`, passive UI status callbacks, `showMapObjectSelectDialog`,
  `showBlockingDialog`, `showTeleportDialog`, and `makeSurrenderRetreatDecision` when `VCMI_NK2_NATIVE_TRACE`
  points at an output JSON file.
...
5. Expand the native `Nullkiller2` trace exporter beyond fixed-answer query, hero-level-up, passive UI status,
   map-object selection, blocking-dialog, teleport-dialog, and surrender/retreat decisions so discrepancies produce
   minimized fixtures under
