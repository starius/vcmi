...
- Planner-accepted target battles now carry the simulation verdict into `ExecuteHeroChain` priority scoring. For those targets, priority scoring uses path-only danger and path-only army loss instead of the static target danger/loss that the simulator just overruled, while still treating the task as battle-related for reward weighting.
