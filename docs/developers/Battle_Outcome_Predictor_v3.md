...
- `BattleSimulationEvaluator` is owned by `BattleProcessor` and exposed through `BattleProcessor::evaluateBattleSimulation`. It currently validates requests and returns exact or composable cached summaries; it returns `NOT_AVAILABLE` until state-isolated repeated simulation populates those summaries.
...
- `BattleSimulationSetup` intentionally returns no setup for an inside siege where a visiting defending hero must first receive merged town-garrison stacks. That merge mutates live state in `CGTownInstance::onHeroVisit`, so the runtime service still needs a state-isolated setup phase before those cases can be simulated safely.
