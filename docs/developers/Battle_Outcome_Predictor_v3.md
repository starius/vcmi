...
- `BattleSimulationSeed` provides stable deterministic sample seed derivation from game seed, player, hero, target, battle type, turn, sample index, and evaluator version.
- `BattleSimulationRequest` and `BattleSimulationResponse` name the future server-owned evaluator boundary: battle setup, deterministic seed context, sample count, thresholds, summary, interpreted evaluation, and small validity/completion checks.
