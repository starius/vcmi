...
- schema6 battle rows add `battleStartStacks`, `battleStartObstacles`, and `battleStartWallState`. `battleStartStacks` is captured after tactics, summons, stack enchantment triggers, and opening spells, before first-round turn selection, and records every non-ghost stack's effective attack/defense/damage/speed, morale/luck, current health, current/initial hex, shooting/casting capability, war-machine/tower flags, and turret aggregates. `battleStartObstacles` records obstacle type/position/affected tiles plus blocking, stopping, trigger, hidden/trap, moat, and spell-created obstacle fields. `battleStartWallState` records the siege wall/gate state at the same post-opening-effects moment, so analysis can distinguish original fortifications from Earthquake/opening-spell-altered battle-start walls. This targets a remaining root-cause gap: schema5 still has base creature stats and army power, but not the effective stack values or battlefield obstacle/wall layout after hero, town, terrain, artifact, tactics, and battle-start effects are applied.
...
- `AI/Nullkiller2/tools/compare_battle_predictors.py` records `runtimeBattleSimulation` totals by model in `summary.json`. Use `--candidate-ai Nullkiller2V3Simulation` and `--require-runtime-simulation candidate` for V3 runtime A/B runs so the script fails if candidate games do not show simulation requests or if too few requests complete. For proof runs, also use `--require-runtime-simulation-configured-samples 20`, `--max-runtime-simulation-incomplete 0`, `--max-runtime-simulation-invalid 0`, `--max-runtime-simulation-not-available 0`, and `--max-runtime-simulation-planning-incomplete 0` so bad or wrong-sample-count simulation evidence fails explicitly. Add `--min-runtime-simulation-town-requests` and `--min-runtime-simulation-planning-town-targets` when the run must prove that the final movement gate and planner-side simulation actually selected town/siege battle targets. Add `--min-runtime-simulation-planning-decisions`, `--min-runtime-simulation-planning-vetoes`, and `--min-runtime-simulation-planning-rescues` when the run must prove that planner-side simulation produced completed verdicts and actually exercised both static false-safe vetoes and static false-unsafe rescues. Add `--min-runtime-simulation-planning-candidates`, `--min-runtime-simulation-planning-decision-rate`, `--min-runtime-simulation-planning-score-adjusted`, and `--min-runtime-simulation-planning-score-positive` when the run must prove planner simulation had enough dangerous-candidate coverage and produced usable positive priority scores. Add `--min-valid-games`, `--max-invalid-paired-samples`, `--min-paired-decisive-samples`, `--max-candidate-better-p`, and `--min-candidate-win-rate-wilson-lower` when the run must prove a statistically strong end-to-end improvement.
...
- Planner stats count simulation skip reasons for dangerous capture candidates: future-turn paths, unsafe intermediate path danger, projected-army paths, and no simulatable battle target. These counters are needed to diagnose why a full-game A/B run may have few simulator-backed planning decisions even when the final movement gate is active.
- Planner-accepted target battles now carry the simulation verdict into `ExecuteHeroChain` priority scoring. For those targets, priority scoring uses path-only danger and path-only army loss instead of the static target danger/loss that the simulator just overruled, while still treating the task as battle-related for reward weighting and using the target danger only as an XP/reward signal.
...
- implementation update: runtime simulation target eligibility is now shared between the final movement gate and capture planning. For v3 with simulation samples enabled, capture planning can recheck same-turn, current-army battle visits if the route's non-target path danger is already statically safe. Completed deterministic repeated simulation verdicts now both rescue static false-unsafe targets and veto static false-safe targets before movement planning picks the goal; future-turn and projected-army chain paths still use static planning because the current simulation API cannot represent their future army state. The A/B tool now parses `planningAccepted`, `planningRejected`, and `planningIncomplete` counters from logs, plus split proof counters for `planningRejectedStaticSafe` vetoes and `planningAcceptedStaticUnsafe` rescues. Use `--min-runtime-simulation-planning-decisions`, `--min-runtime-simulation-planning-vetoes`, and `--min-runtime-simulation-planning-rescues` on the next runtime smoke/A-B run to prove the new planner behavior was actually exercised. Runtime logs and the A/B parser now also expose `townRequests` and `planningTownTargets`, so town/siege proof runs can require that the final movement gate and planner-side simulation actually selected town battle targets.
- implementation update: object clustering now keeps same-turn, current-army town targets for the v3 simulation planner when static priority alone would discard them, provided the existing planner can build a runtime simulation request and the non-target path danger is already acceptable. This is needed for static false-unsafe town/siege rescues to reach capture planning instead of being filtered out during candidate discovery.
- implementation update: the client-side runtime simulation evaluator now checks its deterministic cache before cloning the game state for isolated simulation. This matters for planner-side simulation because capture planning can ask about the same hero/target/sample-count request repeatedly while scanning objects; cached complete responses now avoid the full `cloneGameStateForSimulation` and remap path. Runtime logs and the A/B parser now expose `cacheHits` and `planningCacheHits` to quantify this reuse.
- implementation update: `Nullkiller2V3Simulation` is now a selectable adventure AI alias for A/B tests. It keeps the regular difficulty config intact, forces the V3 battle predictor, enables 20 runtime simulation samples, and uses planner ratio `1.0`; use this alias instead of broad `--config-replace` edits when comparing the simulation-backed candidate against `Nullkiller2`.
...
- refreshed live schema5 snapshot at 208 complete shard groups / 10400 complete-shard rows: static cxx-v3 remained poor on town-hero rows, with 54.33% overall accuracy, Brier 0.3422, 80 false-safe groups, and 14 false-unsafe groups. The held-out split kept 43 groups; fallback-only all-wins simulation reached 95.35% accuracy with 8 samples and 97.67% accuracy with 15/20 samples, with 0 false-safe groups and 1 conservative false-unsafe group at all three sample counts. Static miss segments stayed dominated by visiting-hero sieges, defender spellbooks, defender combat-spell advantage, tavern, citadel/castle, moat, and defender flying-share cases. Deployed-danger factor 2.0 reduced the held-out split to 0 false-safe groups only by allowing 3/43 held-out groups as safe and creating 9 false-unsafe groups, so scalar town danger tuning still is not a viable replacement for simulation-backed decisions.
- refreshed annotated static-miss snapshot at 212 complete shard groups / 10600 complete-shard rows: cxx-v3 remained weak overall at 54.25% accuracy, Brier 0.3447, 81 false-safe groups, and 15 false-unsafe groups. On the held-out split, cxx-v3 was 41.86% accurate with Brier 0.4127, 18 false-safe groups, and 5 false-unsafe groups. Deployed-danger factor 1.0 reached 75.47% overall but still left 24 false-safe groups; factor 1.5 still left 7 false-safe groups; factor 2.0 removed held-out false-safes only by allowing 3 held-out groups as safe and creating 9 false-unsafe groups. Annotated repeated-simulation samples were decisive for the inspected failures: all six close-even cxx-v3 misses and all six worst false-safe groups had `all_wins_safe=false` at 8, 15, and 20 samples, so the simulation gate would block them. The top five inspected false-unsafe groups all had 8/8, 15/15, and 20/20 attacker wins, so planner-side simulation can rescue those static false negatives. One marginal 96% empirical win-rate false-unsafe group showed why 15/20 all-wins is intentionally conservative: 8/8 passed, but 14/15 and 19/20 failed the all-wins rule.
- refreshed schema5 snapshot at 229 complete shard groups / 11450 complete-shard rows: cxx-v3 stayed weak overall at 54.59% accuracy, Brier 0.3381, 89 false-safe groups, and 16 false-unsafe groups. On the held-out split, static cxx-v3 was 39.13% accurate with Brier 0.4272, 20 false-safe groups, and 5 false-unsafe groups. Deployed-danger factor 2.0 removed held-out false-safes only by allowing 3 held-out groups as safe and creating 10 false-unsafe groups. Fallback-only all-wins simulation on the same live data kept clearing the 95% target: 8 samples reached 95.65% held-out accuracy, Brier 0.0148, 0 false-safe groups, and 1 conservative false-unsafe group; 15 and 20 samples reached 97.83% held-out accuracy with Brier 0.0052 and 0.0017, also with 0 false-safe and 1 false-unsafe group. The inspected close-even and worst false-safe groups were still all rejected by 8/15/20 all-wins simulation, while the top five inspected static false-unsafe groups were all rescued by 8/15/20 all-wins simulation; the same marginal 96% empirical group stayed conservative at 15/20 samples.
- refreshed fallback proxy at 239 live shard groups / 12049 parsed rows: static cxx-v3 on the held-out split was 38.30% accurate with Brier 0.4268, 21 false-safe groups, and 5 false-unsafe groups. Fallback-only all-wins simulation remained the best result so far: 8 samples reached 95.74% held-out accuracy, Brier 0.0148, 0 false-safe groups, and 1 conservative false-unsafe group; 15 and 20 samples reached 97.87% held-out accuracy with Brier 0.0052 and 0.0024, also with 0 false-safe and 1 false-unsafe group. Static/simulation hybrids that leave confident static predictions unsimulated remained poor, so the current evidence still favors runtime simulation as the town/siege safety gate.
- refreshed fallback proxy at 261 live shard groups / 13251 parsed rows: static cxx-v3 on the held-out split was 38.89% accurate with Brier 0.4168, 24 false-safe groups, and 5 false-unsafe groups. Fallback-only all-wins simulation reached 96.30% held-out accuracy at 8 samples with Brier 0.0136, but now had 1 false-safe and 1 false-unsafe group. Fifteen samples reached 98.15% accuracy with Brier 0.0049, also with 1 false-safe and 1 false-unsafe group. The cleanest current safety point is 20 samples: 98.15% held-out accuracy, 98.15% safety accuracy, Brier 0.0027, 0 false-safe groups, and 1 conservative false-unsafe group. This keeps simulation-backed all-wins prediction ahead of any static town/siege heuristic tested so far.
