...
- Nullkiller's `battlePredictionSimulationSamples` final movement gate currently calls through `CCallback` to `IClient::evaluateBattleSimulationForVisit`; without an installed provider this returns `NOT_AVAILABLE`. The AI now logs a one-time warning when samples are configured but no complete simulation result is available, so A/B runs do not silently look like they are using runtime simulation.
