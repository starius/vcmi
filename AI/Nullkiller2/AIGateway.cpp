...
struct RuntimeBattleSimulationStats
{
	...
	uint64_t planningAcceptedStaticSafe = 0;
	...
	uint64_t planningScoreAdjusted = 0;
};
...
RuntimeBattleSimulationStats runtimeBattleSimulationStatsSnapshot()
{
	...
	return RuntimeBattleSimulationStats{
		...
		runtimeBattleSimulationCacheHits.load(std::memory_order_relaxed),
		planningStats.accepted,
		planningStats.rejected,
		planningStats.incomplete,
		planningStats.acceptedStaticSafe,
	...
	const RuntimeBattleSimulationStats & before)
{
	return RuntimeBattleSimulationStats{
		...
		after.planningAcceptedStaticSafe - before.planningAcceptedStaticSafe,
		...
		after.planningScoreAdjusted - before.planningScoreAdjusted
	};
}
...
void logRuntimeBattleSimulationStats(PlayerColor playerID, const RuntimeBattleSimulationStats & stats)
{
	if(!stats.requests
		...
		&& !stats.planningAcceptedStaticSafe
		...
		&& !stats.planningScoreAdjusted)
		return;

	logAi->info(
		"Runtime battle simulation stats for player %d (%s): requests %llu, complete %llu, incomplete %llu, safe %llu, rejected %llu, invalid %llu, not available %llu, skipped no target %llu, cache hits %llu, planning accepted %llu, planning rejected %llu, planning incomplete %llu, planning accepted static safe %llu, planning accepted static unsafe %llu, planning rejected static safe %llu, planning rejected static unsafe %llu, planning cache hits %llu, planning skipped future turn %llu, planning skipped unsafe path %llu, planning skipped projected army %llu, planning skipped no target %llu, planning score adjusted %llu",
		playerID,
		playerID.toString(),
		...
		static_cast<unsigned long long>(stats.planningAcceptedStaticSafe),
	...
	EPathNodeAction action)
{
	...
	if(simulation.servedFromCache)
		runtimeBattleSimulationCacheHits.fetch_add(1, std::memory_order_relaxed);
