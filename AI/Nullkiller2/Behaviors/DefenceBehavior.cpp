...
void DefenceBehavior::evaluateDefence(Goals::TGoalVec & tasks, const CGTownInstance * town, const Nullkiller * aiNk) const
{
	...
	{
#if NK2AI_TRACE_LEVEL >= 1
		logAi->trace("Can not plan defence for town %s while it contains a foreign hero.", town->getObjectName());
#endif
		return;
	}
	...
	if(handleGarrisonHeroFromPreviousTurn(town, tasks, aiNk, threats))
		return;
	...
	for(auto & threat : threats)
	{
		...
		AIPath * closestEligiblePath = nullptr;
		...
		for(int i = 0; i < paths.size(); i++)
		{
			...
			if(!closestEligiblePath || path.movementCost() < closestEligiblePath->movementCost())
				closestEligiblePath = &path;
		...
		for(int i : pathsToDefend)
		{
			...
			if(closestEligiblePath)
			{
				heroChain.closestWayRatio = closestEligiblePath->movementCost() / heroChain.getPath().movementCost();
			}
