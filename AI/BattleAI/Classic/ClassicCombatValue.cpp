	...
	bool ranged) const
{
	...
	if(const CGHeroInstance * owner = battle->battleGetFightingHero(view.controllingSide(stack)))
	{
	   ...
	   || vstd::contains(activeSpells, SpellID(SpellID::CURSE)))
	{
		...
		const double adjustedAverage =
			(ClassicRulesAdapter::minDamage(stack, rangedAttack)
			 + ClassicRulesAdapter::maxDamage(stack, rangedAttack)) / 2.0;
		if(baseAverage > 0.0)
