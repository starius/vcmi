	...
	int32_t distance) const
{
	...
	const DamageEstimation estimate = battle->battleEstimateDamage(attackInfo);
	return estimate.damage.min;
}

int64_t ClassicAttackEvaluator::fireShieldDamage(
	const CStack * shieldBearer,
	int64_t shieldBearerHealth,
	const CStack * recipient,
	int64_t incomingDamage) const
{
	...
	if(!fireShield)
		return rawDamage;
	const CGHeroInstance * hero = battle->battleGetFightingHero(shieldBearer->unitSide());
	...
	int64_t defenderHealth) const
{
	...
	if(ranged && !battle->battleCanShoot(attacker, defender->getPosition()))
	{
		...
		const bool friendlyAreaSecondary =
			...
			&& battle->battleCanShoot(attacker);
	...
	if(!ranged)
	{
		result.firstFireShield = fireShieldDamage(
			defender, result.defenderAfter, attacker, result.firstStrike);
	   ...
	   && !blocksRetaliation)
	{
		...
		if(defenderState->ableToRetaliate())
		{
			...
			result.retaliationFireShield = fireShieldDamage(
				attacker, result.attackerAfter, defender, result.retaliation);
	   ...
	   && attacker->getTotalAttacks(ranged) > 1)
	{
		...
		if(!ranged)
		{
			result.secondFireShield = fireShieldDamage(
				defender, result.defenderAfter, attacker, result.secondStrike);
	...
	bool onlyPriorTargets) const
{
	...
	for(const CStack * actor : stacks)
	{
		...
		if(actor == excluded
		   || actor->hasBonusOfType(BonusType::BIND_EFFECT)
...
ClassicProjectedTargets ClassicAttackEvaluator::projectSpellTargets(
	...
	const ClassicCombatParameters & parameters,
	bool recordTrace) const
{
	if(!recordTrace && trace)
	{
		ClassicAttackEvaluator silent(
			battle, randomGenerator, nullptr, preserveLongMoveWait, secondPhase);
		return silent.findProjectedTargets(physicalSide, parameters);
	}
...
) const
{
	...
	const SimulatedAttack simulation = simulateAttack(attacker, defender, attackFrom, ranged, distance);
...
int64_t ClassicAttackEvaluator::attackChange(
	...
	const ClassicProjectedTargets & projection) const
{
	// get_attack_change at 0x0041F530 is only meaningful while the target has
	...
	const int64_t targetHealth = target->getAvailableHealth();
	...
	for(const CStack * projectedAttacker : view.orderedStacks(false, true))
	{
		...
		const int64_t projectedDamage = std::min<int64_t>(
			...
			averageDamage(
				projectedAttacker,
				projectedAttacker->getAvailableHealth(),
				target,
				targetHealth,
...
{
	int64_t result = dangerMap ? dangerAt(attacker, landing, *dangerMap) : 0;
	ClassicBattleStateView view(battle);
	...
	{
		if(!enemy->canMove() || !enemy->isShooter() || !battle->battleCanShoot(enemy))
			continue;
	...
	const ClassicCombatParameters & parameters) const
{
	...
	// In the authenticated six-stack exchange scenario a Normal-difficulty
	// neutral Ancient Behemoth rejects cells 162, 161, and 160 for this reason.
	...
	for(const CStack * enemy : view.orderedEnemies(stack))
	{
		...
		// 0x004204E6..0x00420617 first projects each enemy with a
		// speed-limited combat search. If that stack can already attack one of
		// our current stacks, the executable records it in a separate target
		// mask and does not add its ordinary melee value to this map.
		if(hasImmediateTarget)
			continue;
		...
		// SeedCombatPosition receives speed + 1 as both its limit and base
		// speed at 0x004204E6..0x00420502. Each visited search record paints its
		// occupied cells directly. It is not expanded to neighbouring attack
		// cells: that would let one search record contribute to six unrelated
		// destinations. For a double-wide
		// stack, 0x0042067D..0x00420717 also maps the second occupied cell. The
		// executable's per-enemy cell mask prevents overlapping double-wide
		// records from adding the same enemy's value twice to one map cell.
	...
	const ClassicCombatParameters & parameters) const
{
	...
	for(const CStack * enemy : view.orderedEnemies(stack))
	{
		if(!enemy->alive() || !battle->battleCanShoot(stack, enemy->getPosition()))
			continue;
	...
	const ClassicCombatParameters & parameters) const
{
	...
	const CStack * closedGateTarget = nullptr;
	...
	for(const CStack * enemy : view.orderedEnemies(stack))
	{
		...
		// find_attack_hex first scans the target-relative anchors.  For a
		// double-wide attacker these anchors cannot place both occupied cells;
		// it then performs the extra head-position scan that yields the legal
		// cells exposed by VCMI's getAttackableHexes().  Preserve the rejected
		// first pass in the ordered diagnostic stream as it is observable in
		// the executable and is useful for detecting geometry-order regressions.
		if(trace && stack->doubleWide())
		{
			const int32_t anchorOffset = stack->unitSide() == BattleSide::ATTACKER ? 1 : -1;
			for(const BattleHex & landing : attackable)
			{
				const BattleHex candidate(landing.toInt() + anchorOffset);
				trace->record("attack_hex.candidate", std::to_string(candidate.toInt()), candidate.toInt());
				traceBest(candidate);
			}
			for(int index = 0; index < 3; ++index)
			{
				const BattleHex candidate(BattleHex::INVALID);
				trace->record("attack_hex.candidate", "-1", -1);
				traceBest(candidate);
			}
		}
		...
		for(const BattleHex & landing : attackable)
		{
			...
			const int64_t context = landingContextValue(stack, landing, parameters, &dangerMap);
			if(trace)
			{
				trace->record("attack_hex.time", std::to_string(landing.toInt()), attackTime);
				trace->record("attack_hex.adjacent", std::to_string(landing.toInt()), 0);
				trace->record("attack_hex.adjacent_final", std::to_string(landing.toInt()), 0);
				trace->record("attack_hex.danger", std::to_string(landing.toInt()), context);
		...
		// These are caller guards at 0x00421A87..0x00421B22, not part of
		// get_attack_change's own entry checks.
		...
		const int64_t targetAttackChange = mayChangeProjectedAttack
			...
			: 0;
		const int64_t simpleAttackValue = attackValue(stack, enemy, enemyBestHex, false, parameters);
		...
		{
			// 0x00421CAF..0x00421D2B treats Blind, Stone Gaze, and
			// Paralyze as one target class. An enabled target always replaces a
			// disabled one, while a disabled target can never replace an enabled
			// one, regardless of distance or score.
			select = bestDisabled;
		}
	...
	else
	{
		...
		else if(closedGateTarget)
		{
			// The executable leaves the current enemy pointer populated when
			// the gate-path search fails. It is semantically irrelevant to a
			// DEFEND/WAIT command, but it is observable in the oracle record.
			best.action.target.push_back({
				static_cast<int32_t>(closedGateTarget->unitId()),
				BattleHex::INVALID});
		}
	}
	return best;
...
BattleHex ClassicAttackEvaluator::siegeAdvanceTarget(const CStack * stack) const
{
	...
	// 0x0063BD00: the original wall-contour cell for each battlefield row.
	// choose_melee_target scans left from that byte until it finds a normally
	// occupiable, non-moat cell, then passes that cell to move_toward.
	...
	// Fortress uses a second, wider moat contour in addition to the ordinary
	// one (0x0063BCF4 in the authenticated executable).
	...
	bool enabled) const
{
	...
	for(int32_t index = 0; index < GameConstants::BFIELD_SIZE; ++index)
	{
		...
		for(const BattleHex & neighbour : adjacent)
		{
			...
			if(!occupant || occupant == stack)
				++score;
			else if(occupant->isShooter())
				score = 1000;
		}

		if(score > bestScore
			|| (score == bestScore && candidate != stack->getPosition()))
		{
			bestScore = score;
			bestHex = candidate;
		}
	...
	const ClassicCombatParameters & parameters) const
{
	...
	if(result.valid)
		return result;
	const auto staleTarget = result.action.target;
	...
	result.action = stack->waited()
		...
		? BattleAction::makeDefend(stack)
		: BattleAction::makeWait(stack);
	result.action.target = staleTarget;
	return result;
}
