/*
 * Replay.cpp, part of VCMI engine
 *
...
class ScopedNonGameplayStateNormalization final
{
	std::vector<std::pair<PlayerState *, JsonNode>> savedSettings;
	std::optional<StatisticDataSet> savedStatistics;
	StatisticDataSet * savedStatisticsTarget = nullptr;
	struct SavedPlayerSettings
	{
		PlayerSettings * settings;
		std::string name;
		std::set<PlayerConnectionID> connections;
	};
	std::vector<SavedPlayerSettings> savedPlayerSettings;

public:
	explicit ScopedNonGameplayStateNormalization(CGameState & gameState, StatisticDataSet * statistics = nullptr)
	{
		savedSettings.reserve(gameState.players.size());
		for(auto & player : gameState.players)
		{
			savedSettings.emplace_back(&player.second, *player.second.playerLocalSettings);
			*player.second.playerLocalSettings = JsonNode();
		}
		for(StartInfo * startInfo : {gameState.getStartInfo(), gameState.getInitialStartInfo()})
		{
			if(!startInfo)
				continue;
			for(auto & entry : startInfo->playerInfos)
			{
				auto & settings = entry.second;
				savedPlayerSettings.push_back({&settings, settings.name, settings.connectedPlayerIDs});
				settings.name.clear();
		...
		if(statistics)
		{
			savedStatisticsTarget = statistics;
			savedStatistics = std::move(*statistics);
			*statistics = StatisticDataSet();
		}
	}

	~ScopedNonGameplayStateNormalization()
	{
		for(auto & saved : savedSettings)
			*saved.first->playerLocalSettings = std::move(saved.second);
		for(auto & saved : savedPlayerSettings)
		{
			saved.settings->name = std::move(saved.name);
			saved.settings->connectedPlayerIDs = std::move(saved.connections);
		}
		if(savedStatistics)
			*savedStatisticsTarget = std::move(*savedStatistics);
	}
};
...
class ReplayGameServer : public IGameServer
{
	...
	std::vector<std::byte> serializeTurnState(CGameHandler & handler) const
	{
		ScopedNonGameplayStateNormalization normalized(*handler.gs, handler.statistics.get());
		CSaveFile save;
		handler.gameState().saveGame(save);
		save.save(handler);
		return save.currentContent();
	}

	std::vector<std::byte> serializeExpectedTurnState(const boost::filesystem::path & path)
	{
		if(!boost::filesystem::is_regular_file(path))
			throw std::runtime_error("Missing expected VGT turn state: " + path.string());

		CGameHandler expectedHandler(*this);
		expectedHandler.gs = std::make_shared<CGameState>();
		expectedHandler.gs->preInit(LIBRARY);
		expectedHandler.randomizer = std::make_unique<GameRandomizer>(*expectedHandler.gs);

		CLoadFile loadFile(path, expectedHandler.gs.get());
		expectedHandler.gs->loadGame(loadFile);
		loadFile.load(expectedHandler);
		return serializeTurnState(expectedHandler);
	}
	...
	std::vector<std::byte> canonicalizeSerializedTurnState(const std::vector<std::byte> & data)
	{
		const auto temporaryPath = boost::filesystem::temp_directory_path() /
			boost::filesystem::unique_path("vcmi-vgt-turn-state-%%%%%%%%.vsgm1");
		try
		{
			writeTurnState(temporaryPath, data);
			auto result = serializeExpectedTurnState(temporaryPath);
			boost::system::error_code removeError;
			boost::filesystem::remove(temporaryPath, removeError);
			return result;
		}
		catch(...)
		{
	...
	void processTurnState(PlayerColor player)
	{
		...
		const auto actual = serializeTurnState(*gameHandler);
		...
		auto expected = readTurnState(expectedPath);
		if(actual == expected)
		{
		...
		auto canonicalActual = canonicalizeSerializedTurnState(actual);
		auto canonicalExpected = serializeExpectedTurnState(expectedPath);
		if(canonicalActual == canonicalExpected)
		{
			logGlobal->info(
				"VGT turn state matches '%s' after canonicalizing non-gameplay player settings",
				expectedPath.string());
			return;
		}
		expected = std::move(canonicalExpected);
		const auto & comparedActual = canonicalActual;
		if(const char * mismatchPath = std::getenv("VCMI_VGT_REPLAY_MISMATCH_SAVE"); mismatchPath && *mismatchPath)
			writeTurnState(boost::filesystem::path(mismatchPath), comparedActual);

		const size_t commonSize = std::min(comparedActual.size(), expected.size());
		size_t firstDifference = 0;
		while(firstDifference < commonSize && comparedActual[firstDifference] == expected[firstDifference])
			++firstDifference;

		std::ostringstream message;
		message << "VGT turn state mismatch: " << expectedPath.string()
			<< ", expected size " << expected.size()
			<< ", replayed size " << comparedActual.size()
		...
		if(firstDifference < commonSize)
		{
			message << " (expected " << std::to_integer<unsigned>(expected[firstDifference])
				<< ", replayed " << std::to_integer<unsigned>(comparedActual[firstDifference]) << ")";
		}
...
std::string objectAliasIdentifier(std::string identifier)
{
...
{
	return objectAliasIdentifier(MapObjectID::encode(id.getNum()));
}
...
std::string canonicalObjectAlias(const CGObjectInstance & object)
{
	...
	std::string result;
	if(generatedObjectAliasName(name, typeName, object.id))
	{
		result = type;
		if(!owner.empty())
			result += "/" + owner;
...
int normalizeVGTGameStateSave(const VGTGameStateNormalizeOptions & options)
{
	...
	ScopedNonGameplayStateNormalization normalized(gameState);
	CSaveFile save;
...
{
	VGTRecorder::get().suppressAutomaticRecording();
	...
	if(hasField(header, "partial"))
		throw std::runtime_error("A partial VGT transcript cannot replay until its missing history is repaired");
	...
	if(documents.Vector().size() > 1)
	{
		...
		for(const auto & entry : startInfo.playerInfos)
		{
			AdvInterfaceReady ready;
			replayPack(gameHandler, ready, entry.first);
		}
	}
