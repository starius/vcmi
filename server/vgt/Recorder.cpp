/*
 * Recorder.cpp, part of VCMI engine
 *
...
bool generatedObjectAliasName(const std::string & name, const std::string & type, ObjectInstanceID id)
{
...
std::string objectAlias(const CGameState & gameState, ObjectInstanceID id)
{
	...
	if(const auto * monster = dynamic_cast<const CGCreature *>(object))
		return "monster/" + pluralizedBattleCreature(monster->getCreatureID()) + objectLocation(object->visitablePos());
	...
	std::string result;
	if(generatedObjectAliasName(name, typeName, id))
	{
		result = type;
		if(!owner.empty())
			result += owner;
	}
	else if(equivalentAliasWords(name, typeName))
...
std::string playerSettings(const PlayerSettings & value, const PlayerSettings * base = nullptr)
{
	...
	constexpr size_t HUMAN_PLAYER_NAME_TEXT_INDEX = 434;
	constexpr size_t COMPUTER_PLAYER_NAME_TEXT_INDEX = 468;
	const std::string localizedDefaultName = LIBRARY->generaltexth->allTexts[
		human ? HUMAN_PLAYER_NAME_TEXT_INDEX : COMPUTER_PLAYER_NAME_TEXT_INDEX];
	const bool generatedDefaultName = value.name.empty() || value.name == localizedDefaultName ||
		(human ? value.name == "Player" : value.name == "Computer");
	if(changed(value.name, base ? base->name : std::string()) && !generatedDefaultName)
...
std::optional<std::string> hashFile(const boost::filesystem::path & filePath)
{
...
std::string utcTimestamp(std::time_t value)
{
	std::tm time = {};
#ifdef _WIN32
	gmtime_s(&time, &value);
...
bool isGeneratedTimestampPrefix(const std::string & value)
{
	if(value.size() < 16 || value[8] != 'T' || value[15] != '_')
		return false;
...
std::string transcriptMapSlug(const StartInfo & startInfo)
{
	std::string source = boost::filesystem::path(startInfo.fileURI).stem().string();
	if(isGeneratedTimestampPrefix(source))
		source.erase(0, 16);
...
boost::filesystem::path uniqueTranscriptPath(const boost::filesystem::path & directory, const std::string & stem)
{
	boost::filesystem::path result = directory / (stem + ".vgt");
	for(int suffix = 2; boost::filesystem::exists(result); ++suffix)
	{
		std::ostringstream numbered;
		numbered << stem << "_" << std::setfill('0') << std::setw(2) << suffix << ".vgt";
		result = directory / numbered.str();
	}
	return result;
}
...
public:
	...
	void visitSaveLocalState(SaveLocalState &) override
	{
		line.clear();
	}
	...
	void visitPlayerMessage(PlayerMessage &) override
	{
		// Chat and generated UI notices (including quicksave feedback) are not game history.
		line.clear();
	}
...
public:
	...
	void visitPlayerMessageClient(PlayerMessageClient &) override
	{
		// Player chat and generated UI notices do not modify authoritative game state.
		line.clear();
	}
...
namespace
{
constexpr std::string_view SAVEPOINT_MARKER = "# vgt-savepoint: ";

struct SavepointMatch
{
	boost::filesystem::path transcriptPath;
	uint64_t transcriptOffset = 0;
	uint64_t battleOffset = 0;
	uint64_t nextBattleID = 0;
	std::string gameID;
	std::optional<PlayerColor> player;
	bool documentOpen = false;
	bool atTranscriptEnd = false;
	bool trimTranscriptTail = false;
};

uint64_t nextBattleIDInTranscript(const boost::filesystem::path & transcriptPath, uint64_t offset)
{
	std::ifstream input(transcriptPath.string(), std::ios::binary);
	if(!input)
		return 0;
	...
	uint64_t position = 0;
	std::string line;
	while(position < offset && std::getline(input, line))
	{
		position += line.size() + 1;
		if(!line.starts_with(ID_PREFIX))
			continue;
		const std::string value = line.substr(ID_PREFIX.size());
		...
		try
		{
			result = std::max<uint64_t>(result, std::stoull(value) + 1);
		}
		catch(const std::exception &)
		{
		}
...
bool isLifecycleOnlyTail(const boost::filesystem::path & transcriptPath, uint64_t offset)
{
	std::ifstream input(transcriptPath.string(), std::ios::binary);
	if(!input)
		return false;
	...
	while(std::getline(input, line))
	{
		...
		if(line.empty())
			continue;
		if(line == "- pauseTimer" || line == "- answer: { value: 0 }")
			continue;
		if(line == "- ready" || line.starts_with("- ready: { actor: "))
			continue;
		return false;
	}
	return true;
...
std::optional<SavepointMatch> findSavepointInTranscript(
	const boost::filesystem::path & transcriptPath,
	const std::string & saveHash)
{
	...
	const uint64_t transcriptSize = boost::filesystem::file_size(transcriptPath);
	uint64_t offset = 0;
	std::optional<SavepointMatch> result;
	std::string line;
	while(std::getline(input, line))
	{
		offset += line.size() + 1;
		const auto markerPosition = line.find(SAVEPOINT_MARKER);
		if(markerPosition == std::string::npos)
			continue;

		const std::string json = line.substr(markerPosition + SAVEPOINT_MARKER.size());
		try
		{
			const JsonNode marker(json.data(), json.size(), transcriptPath.string());
			if(!marker.isStruct() || !marker["saveSha256"].isString() ||
				marker["saveSha256"].String() != saveHash || !marker["battleOffset"].isNumber())
				continue;

			SavepointMatch match;
			match.transcriptPath = transcriptPath;
			match.transcriptOffset = std::min(offset, transcriptSize);
			const int64_t battleOffset = marker["battleOffset"].Integer();
			if(battleOffset < 0)
				continue;
			match.battleOffset = static_cast<uint64_t>(battleOffset);
			if(marker["nextBattleId"].isNumber() && marker["nextBattleId"].Integer() >= 0)
				match.nextBattleID = static_cast<uint64_t>(marker["nextBattleId"].Integer());
			else
				match.nextBattleID = nextBattleIDInTranscript(transcriptPath, match.transcriptOffset);
			match.documentOpen = marker["documentOpen"].Bool();
			match.atTranscriptEnd = match.transcriptOffset == transcriptSize;
			if(!match.atTranscriptEnd && isLifecycleOnlyTail(transcriptPath, match.transcriptOffset))
			{
				match.atTranscriptEnd = true;
				match.trimTranscriptTail = true;
			}
			if(marker["gameId"].isString())
				match.gameID = marker["gameId"].String();
			if(marker["player"].isString())
			{
				const int decoded = PlayerColor::decode(marker["player"].String());
				if(decoded >= 0 && decoded < PlayerColor::PLAYER_LIMIT_I)
					match.player = PlayerColor(decoded);
			}
			result = std::move(match);
		}
		catch(const std::exception & error)
		{
			logGlobal->warn("Ignoring malformed VGT savepoint in '%s': %s", transcriptPath.string(), error.what());
		}
	}
	return result;
}

std::optional<SavepointMatch> findSavepoint(
	const boost::filesystem::path & transcriptDirectory,
	const std::string & saveHash)
{
	if(!boost::filesystem::is_directory(transcriptDirectory))
		return std::nullopt;

	std::vector<boost::filesystem::path> transcripts;
	for(const auto & entry : boost::filesystem::directory_iterator(transcriptDirectory))
	{
		if(boost::filesystem::is_regular_file(entry.path()) && entry.path().extension() == ".vgt")
			transcripts.push_back(entry.path());
	}
	std::sort(transcripts.begin(), transcripts.end());

	std::optional<SavepointMatch> best;
	for(const auto & transcript : transcripts)
	{
		auto match = findSavepointInTranscript(transcript, saveHash);
		if(!match)
			continue;
		const boost::filesystem::path battlePath(transcript.string() + ".battles.yaml");
		if(!boost::filesystem::is_regular_file(battlePath) || boost::filesystem::file_size(battlePath) < match->battleOffset)
			continue;
		if(boost::filesystem::file_size(battlePath) > match->battleOffset)
		{
			match->atTranscriptEnd = false;
			match->trimTranscriptTail = false;
		}
		if(!best || (match->atTranscriptEnd && !best->atTranscriptEnd))
			best = std::move(match);
	}
	return best;
}

void copyPrefix(std::istream & input, std::ostream & output, uint64_t size)
{
	std::array<char, 64 * 1024> buffer = {};
	while(size > 0)
	{
		const auto chunk = static_cast<std::streamsize>(std::min<uint64_t>(size, buffer.size()));
		input.read(buffer.data(), chunk);
		const auto read = input.gcount();
		if(read <= 0)
			throw std::runtime_error("Unexpected end of VGT file while copying a branch prefix");
	...
	uint64_t size)
{
	...
	if(!input || !output)
		throw std::runtime_error("Unable to create VGT branch file '" + destination.string() + "'");
	copyPrefix(input, output, size);
}

void copyBattlePrefix(
	const boost::filesystem::path & source,
	const boost::filesystem::path & destination,
	uint64_t size,
	const std::string & mainFileName)
{
	...
	std::ofstream output(destination.string(), std::ios::binary | std::ios::trunc);
	if(!input || !output)
		throw std::runtime_error("Unable to create VGT battle branch file '" + destination.string() + "'");

	std::string firstLine;
	std::string mainLine;
	if(!std::getline(input, firstLine) || !std::getline(input, mainLine))
		throw std::runtime_error("VGT battle companion has an incomplete header: " + source.string());
	const uint64_t consumed = firstLine.size() + mainLine.size() + 2;
	if(consumed > size)
		throw std::runtime_error("VGT battle savepoint precedes the companion header: " + source.string());

	output << firstLine << '\n';
	output << "main: " << yamlString(mainFileName) << '\n';
	copyPrefix(input, output, size - consumed);
}
}
...
void VGTRecorder::resetGame()
{
	...
	activeGameHandler = nullptr;
	outputPath.clear();
	battleOutputPath.clear();
	gameID.clear();
	partialSaveHash.clear();
	enabled = false;
	partialTranscript = false;
	headerWritten = false;
	documentOpen = false;
	exitAfterAppliedState = false;
	randomSeed.reset();
	observedTurnEnds = 0;
	archivedTurnStates = 0;
	currentTurnPlayer.reset();
	resumedOpenTurnPlayer.reset();
	pendingTurnStatePlayer.reset();
	pendingBattle.reset();
	nextBattleTranscriptID = 0;
	lastBattleDecisions.clear();
	pendingMove.reset();
	...
	latestTimerStates.clear();
	turnStartTimerStates.clear();
	discoveryTracker = VGTDiscoveryTracker();
}

void VGTRecorder::flushPendingRecords(const CGameState & gameState)
{
...
void VGTRecorder::suppressAutomaticRecording()
{
...
void VGTRecorder::beginGame(CGameHandler & gameHandler, int seed)
{
	std::scoped_lock lock(outputMutex);
	resetGame();
	...
	activeGameHandler = &gameHandler;
	randomSeed = seed;
	enabled = true;
	ensureHeader(gameHandler.gameState());
}

void VGTRecorder::resumeGame(CGameHandler & gameHandler, const std::string & savePath)
{
	std::scoped_lock lock(outputMutex);
	resetGame();
	if(recordingSuppressed)
	...
	const auto saveHash = hashFile(savePath);
	const boost::filesystem::path transcriptDirectory = VCMIDirs::get().userDataPath() / "Transcripts";
	boost::filesystem::create_directories(transcriptDirectory);
	if(saveHash)
	{
		if(auto match = findSavepoint(transcriptDirectory, *saveHash))
		{
			boost::filesystem::path transcriptPath = match->transcriptPath;
			boost::filesystem::path battlePath(transcriptPath.string() + ".battles.yaml");
			if(!match->atTranscriptEnd)
			{
				const std::string stem = transcriptPath.stem().string() + "_branch-" + utcTimestamp(std::time(nullptr));
				const boost::filesystem::path branchPath = uniqueTranscriptPath(transcriptDirectory, stem);
				const boost::filesystem::path branchBattlePath(branchPath.string() + ".battles.yaml");
				copyFilePrefix(transcriptPath, branchPath, match->transcriptOffset);
				copyBattlePrefix(battlePath, branchBattlePath, match->battleOffset, branchPath.filename().string());
				logGlobal->info(
					"VGT save branches from '%s'; preserving later history and continuing in '%s'",
					transcriptPath.string(), branchPath.string());
				transcriptPath = branchPath;
				battlePath = branchBattlePath;
			}
			else
			{
				if(match->trimTranscriptTail)
				{
					boost::filesystem::resize_file(transcriptPath, match->transcriptOffset);
					logGlobal->info(
						"Discarded non-gameplay VGT lifecycle records after the savepoint in '%s'",
						transcriptPath.string());
				}
				logGlobal->info("Resuming VGT transcript '%s'", transcriptPath.string());
			}

			outputPath = transcriptPath.string();
			battleOutputPath = battlePath.string();
			output.open(outputPath, std::ios::out | std::ios::app);
			battleOutput.open(battleOutputPath, std::ios::out | std::ios::app);
			if(output && battleOutput)
			{
				gameID = match->gameID;
				headerWritten = true;
				documentOpen = match->documentOpen;
				currentTurnPlayer = match->player;
				nextBattleTranscriptID = match->nextBattleID;
				if(documentOpen)
					resumedOpenTurnPlayer = match->player;
				return;
			}
			logGlobal->error("Unable to reopen matching VGT transcript '%s'", outputPath);
			resetGame();
			activeGameHandler = &gameHandler;
			enabled = true;
		}
	...
	partialSaveHash = saveHash.value_or("unavailable");
	randomSeed = gameHandler.randomizer ? gameHandler.randomizer->getDefaultSeed() : 0;
	ensureHeader(gameHandler.gameState());
	if(enabled)
		logGlobal->warn("No matching VGT savepoint was found; recording a partial transcript in '%s'", outputPath);
}

void VGTRecorder::finishGame(CGameHandler & gameHandler)
{
	std::scoped_lock lock(outputMutex);
	if(activeGameHandler != &gameHandler)
		return;
	if(enabled && headerWritten)
		flushPendingRecords(gameHandler.gameState());
	resetGame();
}

void VGTRecorder::prepareSave(CGameHandler & gameHandler)
{
	std::scoped_lock lock(outputMutex);
	if(!enabled || activeGameHandler != &gameHandler)
		return;
	ensureHeader(gameHandler.gameState());
	if(!enabled)
		return;
...
void VGTRecorder::recordSave(CGameHandler & gameHandler, const std::string & savePath)
{
	std::scoped_lock lock(outputMutex);
	if(!enabled || activeGameHandler != &gameHandler || !headerWritten)
		return;
	const auto saveHash = hashFile(savePath);
	if(!saveHash)
	{
		logGlobal->error("Unable to hash saved game '%s' for VGT continuation", savePath);
		return;
	}

	battleOutput.flush();
	const auto battleOffset = battleOutput.tellp();
	if(battleOffset < 0)
	{
		logGlobal->error("Unable to determine VGT battle offset for saved game '%s'", savePath);
		return;
	}

	JsonNode marker;
	marker["saveSha256"].String() = *saveHash;
	marker["gameId"].String() = gameID;
	marker["date"].String() = calendarDate(gameHandler.gameState().getCalendar());
	marker["battleOffset"].Integer() = static_cast<int64_t>(static_cast<std::streamoff>(battleOffset));
	marker["nextBattleId"].Integer() = static_cast<int64_t>(nextBattleTranscriptID);
	marker["documentOpen"].Bool() = documentOpen;
	if(currentTurnPlayer)
		marker["player"].String() = currentTurnPlayer->toString();
	output << (documentOpen ? "  " : "") << SAVEPOINT_MARKER << jsonCompact(marker) << '\n';
	output.flush();
}
...
void VGTRecorder::ensureHeader(const CGameState & gameState)
{
	...
	identity << *hash << '\n'
		<< static_cast<int64_t>(startInfo->startTime) << '\n'
		<< *randomSeed << '\n'
		<< static_cast<int>(startInfo->difficulty) << '\n';
	for(const auto & player : startInfo->playerInfos)
		identity << color(player.first) << ':' << playerSettings(player.second) << '\n';
	if(startInfo->mapGenOptions)
		identity << randomMapGenerator(*startInfo->mapGenOptions) << '\n';
	if(partialTranscript)
		identity << partialSaveHash << '\n';
	const std::string identityText = identity.str();
	gameID = "g" + sha256(
		reinterpret_cast<const uint8_t *>(identityText.data()), identityText.size()).substr(0, 12);

	const boost::filesystem::path transcriptDirectory = VCMIDirs::get().userDataPath() / "Transcripts";
	boost::filesystem::create_directories(transcriptDirectory);
	std::string stem = utcTimestamp(startInfo->startTime) + "_" + transcriptMapSlug(*startInfo) + "_" + gameID;
	if(partialTranscript)
		stem += "_partial-" + utcTimestamp(std::time(nullptr));
	const boost::filesystem::path transcriptPath = uniqueTranscriptPath(transcriptDirectory, stem);
	outputPath = transcriptPath.string();
	...
	if(!output || !battleOutput)
	{
		logGlobal->error("Unable to create VGT transcript pair '%s'", outputPath);
		enabled = false;
		return;
	}
	...
	if(partialTranscript)
		battleOutput << "partial: true\n";
	battleOutput << "battles:\n";
	battleOutput.flush();
	logGlobal->info("Writing VGT transcript '%s'", outputPath);
	...
	if(partialTranscript)
	{
		output << "partial: { reason: missingSavepoint";
		if(partialSaveHash != "unavailable")
			output << ", sourceSaveSha256: " << yamlString(partialSaveHash);
		output << " }\n";
	}
...
void VGTRecorder::discardPendingBattle(bool reuseTranscriptID)
{
	if(!pendingBattle)
		return;
	if(!pendingBattle->runtimeID.empty())
		battleUnitAliases.erase(std::stoi(pendingBattle->runtimeID));
	if(reuseTranscriptID && nextBattleTranscriptID > 0 &&
		pendingBattle->id == std::to_string(nextBattleTranscriptID - 1))
		--nextBattleTranscriptID;
	pendingBattle.reset();
}
...
void VGTRecorder::flushPendingBattle(const CGameState & gameState)
{
	...
	const auto hasOutcome = [this](const std::string & prefix)
	{
		return std::ranges::any_of(pendingBattle->outcome, [&prefix](const std::string & field)
		{
			return field.starts_with(prefix);
		});
	};
	if(pendingBattle->initialRandom.empty() || !pendingBattle->resultRecorded ||
		!hasOutcome("winner: ") || !hasOutcome("loser: ") ||
		pendingBattle->randomBeforeContinuation.empty() || !pendingBattle->armiesCaptured)
	{
		logGlobal->error("Discarding incomplete VGT battle %s instead of writing an invalid block", pendingBattle->id);
		discardPendingBattle(true);
		return;
	}
	...
	{
		output << "        random:\n";
		output << "          beforeContinuation: " << pendingBattle->randomBeforeContinuation << "\n";
		if(!pendingBattle->randomAtContinuation.empty() &&
			pendingBattle->randomAtContinuation != pendingBattle->randomBeforeContinuation)
			output << "          atContinuation: " << pendingBattle->randomAtContinuation << "\n";
	}
...
void VGTRecorder::writeActionLine(const CGameState & gameState, const std::string & line)
{
	...
	{
		flushPendingHeroScene();
		if(!pendingBattle || pendingBattle->runtimeID != battleRecord->battleID)
		{
			flushPendingBattle(gameState);
			pendingBattle.emplace();
			pendingBattle->runtimeID = battleRecord->battleID;
			pendingBattle->id = std::to_string(nextBattleTranscriptID++);
...
void VGTRecorder::recordDecision(CGameHandler & gameHandler, CPackForServer & pack)
{
	...
	if(pendingBattle && pendingBattle->ended && !pendingBattle->aftermathDecisionStarted && gameHandler.randomizer)
		pendingBattle->randomAtContinuation = singleLineJson(gameHandler.randomizer->toVGTBattleJson(
...
void VGTRecorder::recordEffect(CGameHandler & gameHandler, CPackForClient & pack)
{
	...
	if(!enabled)
		return;
	if(auto * cancelled = dynamic_cast<BattleCancelled *>(&pack); cancelled && pendingBattle &&
		pendingBattle->runtimeID == battleAlias(cancelled->battleID))
	{
		lastBattleDecisions.erase(pendingBattle->runtimeID);
		for(auto query = pendingQueries.begin(); query != pendingQueries.end();)
		{
			if(query->second.kind == "battleChoice")
	...
	if(auto * changed = dynamic_cast<BattleUnitsChanged *>(&pack))
	{
		for(const auto & change : changed->changedStacks)
		{
			...
			battle::UnitInfo info;
			info.load(change.id, change.data);
			registerBattleUnitAlias(changed->battleID, static_cast<int>(change.id), info.side, info.type);
		}
	}
	...
	{
		pendingBattle->resultRecorded = true;
		...
		if(gameHandler.randomizer)
			pendingBattle->randomBeforeContinuation = singleLineJson(gameHandler.randomizer->toVGTBattleJson(
				pendingBattle->randomizerParticipants, pendingBattle->randomizerHeroes));
	...
	if(auto * visit = dynamic_cast<HeroVisit *>(&pack); visit && visit->starting)
	{
		...
		encounter.objectID = visit->objId;
	...
	if(auto * dialog = dynamic_cast<BlockingDialog *>(&pack); dialog && pendingBattle &&
		(pendingBattle->ended || pendingBattle->resultRecorded))
	{
	...
	{
		const auto * encounterObject = pendingEncounter
			? gameState.getMap().getObject(pendingEncounter->objectID)
			: nullptr;
		const bool standardShrineText = encounterObject && (
			encounterObject->ID == Obj::SHRINE_OF_MAGIC_INCANTATION ||
			encounterObject->ID == Obj::SHRINE_OF_MAGIC_GESTURE ||
			encounterObject->ID == Obj::SHRINE_OF_MAGIC_THOUGHT);
	...
	if(auto * start = dynamic_cast<PlayerStartsTurn *>(&pack))
	{
		...
		if(resumedOpenTurnPlayer && *resumedOpenTurnPlayer == start->player)
			resumedOpenTurnPlayer.reset();
		else
		{
			resumedOpenTurnPlayer.reset();
