/*
 * Recorder.h, part of VCMI engine
 *
...
class VGTRecorder final
{
	...
	const CGameHandler * activeGameHandler = nullptr;
	std::string outputPath;
	std::string battleOutputPath;
	std::string gameID;
	std::string partialSaveHash;
	...
	std::optional<PlayerColor> resumedOpenTurnPlayer;
	...
	uint64_t nextBattleTranscriptID = 0;
	...
	void resetGame();
	void flushPendingRecords(const CGameState & gameState);
...
public:
	...
	void suppressAutomaticRecording();
	void beginGame(CGameHandler & gameHandler, int seed);
	void resumeGame(CGameHandler & gameHandler, const std::string & savePath);
	void finishGame(CGameHandler & gameHandler);
	void prepareSave(CGameHandler & gameHandler);
	void recordSave(CGameHandler & gameHandler, const std::string & savePath);
