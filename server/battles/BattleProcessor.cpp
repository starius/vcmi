...
void BattleProcessor::updateBattleStartSnapshot(const CBattleInfoCallback & battle)
{
	const auto * info = battle.getBattle();
	if(!info)
		return;

	battleStartSnapshots[info->getBattleID()] = makeBattleStartStateSnapshot(*info);
}
