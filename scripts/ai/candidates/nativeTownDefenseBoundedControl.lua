...
--[[
Native town-defense bounded Nullkiller control
----------------------------------------------
...
The added policy is a narrow emergency-town-defense opening. When the host
reports a critical defense alert and the scripted player has no practical map
tempo left, the script first asks the host to run a bounded native
DEFEND_TOWN subroutine for the threatened town. If the native helper has no
matching executable task, the script falls back to exactly one checked
recruit/build option before the next native slice.

This is deliberately conservative:

* It uses numeric threat levels and town/source ids, not localized labels.
* It asks C++ to match actual DEFEND_TOWN goals instead of parsing task names.
...
losses without promoting the previously rejected unconditional defense pass or
the too-broad Lua-side defense-task selector.
]]
...
local function tryNativeTownDefense(ai, current, memory, alerts)
    for _, alert in ipairs(alerts) do
        local townId = tonumber(alert.town_id or alert.townId)
        if townId then
            local result = ai:nullkillerDefendTown(townId, {
                max_candidates = DefaultMaxCandidates,
                max_attempts = DefaultMaxAttempts
            })
            if result.didExecute == true then
                memory.totalEmergencyDefenseActions = memory.totalEmergencyDefenseActions + 1
                memory.lastIntent = "emergency native town defense"
                memory.lastNativeDefenseTownId = townId
                ai:setMemory(memory)
...
local function runEmergencyDefense(ai, current, memory)
    ...
    local defended
    current, defended = tryNativeTownDefense(ai, current, memory, alerts)
    if defended then
        return current, true
    end
