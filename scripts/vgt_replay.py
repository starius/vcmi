...
def expand_battle_companion(
    ...
    allowed_companion_fields = {"vgtBattles", "main", "battles", "partial"}
    if not set(companion) <= allowed_companion_fields:
        raise VGTError(f"{companion_path} has unsupported top-level fields")
    if "partial" in companion and companion["partial"] is not True:
        raise VGTError(f"{companion_path} has an invalid partial marker")
    ...
    for index, entry in enumerate(battles):
        ...
        if battle_id <= previous_id:
            raise VGTError(f"battle companion entry {index} is not in monotonic id order")
        by_id[battle_id] = battle
        previous_id = battle_id
    ...
    for document in documents[1:]:
        ...
        for record in copy[record_key]:
            ...
            if reference <= previous_id:
                raise VGTError(f"tactical battle reference {reference} is not in monotonic id order")
            merged = dict(battle)
...
def header(documents: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not isinstance(game, dict) or not isinstance(game.get("id"), str):
        raise VGTError("header game.id field is missing or invalid")
...
def select_through_turn(
    documents: list[dict[str, Any]], turn_limit: int
) -> list[dict[str, Any]]:
    """Keep the first N player turns and their immediately following world events."""
    selected = [documents[0]]
    selected_turns = 0
    for document in documents[1:]:
        is_turn = set(document) == {"turn", "actions"}
        if is_turn and selected_turns == turn_limit:
            break
        selected.append(document)
        if is_turn:
            selected_turns += 1

    if selected_turns != turn_limit:
        raise VGTError(
            f"--through-turn {turn_limit} exceeds transcript turn count {selected_turns}"
        )
    return selected
...
def command_replay(args: argparse.Namespace) -> int:
    ...
    if args.schema:
        validate_schema(documents, args.schema)
    transcript_header = header(documents)
    if "partial" in transcript_header:
        raise VGTError("a partial transcript cannot replay until its missing history is repaired")
    if args.resource_root:
        validate_map_hash(transcript_header["map"], args.resource_root)
    ...
    if args.header_only:
        replay_documents = [transcript_header]
    elif args.through_turn is not None:
        replay_documents = select_through_turn(documents, args.through_turn)
    else:
        replay_documents = documents
...
def build_parser() -> argparse.ArgumentParser:
    ...
    extent = replay.add_mutually_exclusive_group()
    extent.add_argument("--header-only", action="store_true", help="rebuild only the initialized state from the transcript header")
    extent.add_argument(
        "--through-turn",
        type=int,
        metavar="N",
        help="replay through the Nth player-turn document, including its following world events",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        if getattr(args, "through_turn", None) is not None and args.through_turn < 1:
            raise VGTError("--through-turn must be a positive integer")
