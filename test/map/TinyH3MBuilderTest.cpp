...
TEST(TinyH3MBuilderTest, TownGarrisonRoundTrips)
{
	...
	auto bytes = TinyH3M::TinyH3MBuilder(EMapFormat::SOD)
		...
		.buildAndDump("TownGarrisonRoundTrips");
	...
	EXPECT_EQ(town->getOwner(), PlayerColor(1));
	EXPECT_EQ(town->visitablePos(), int3(4, 5, 0));
	EXPECT_EQ(town->getStackCount(SlotID(0)), 100);
	EXPECT_EQ(town->getStack(SlotID(0)).getCreatureID(), pikeman);
}
