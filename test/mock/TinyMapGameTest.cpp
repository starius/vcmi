...
{
	auto playerState = gameState->players.find(player);
	ASSERT_NE(playerState, gameState->players.end()) << "Player " << player.getNum() << " not initialised";
	playerState->second.resources[resource] += amount;
}
