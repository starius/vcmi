...
TinyH3MBuilder & TinyH3MBuilder::townGarrison(std::vector<std::pair<CreatureID, uint16_t>> stacks)
{
	...
	assert((spec.id == Obj::TOWN || spec.id == Obj::RANDOM_TOWN)
		&& "townGarrison() must follow town() or randomTown()");
	if(stacks.size() > GameConstants::ARMY_SIZE)
		stacks.resize(GameConstants::ARMY_SIZE);
...
void TinyH3MBuilder::writeObjects(TinyH3MWriter & w) const
{
	...
	for(const auto & obj : objects)
	{
		...
		switch(obj.id.toEnum())
		{
			...
			case Obj::TOWN:
				// No events or custom name; fortification and garrison are fixture-controlled.
				if(features.levelAB)
					w.writeUInt32(obj.wireIdentifier);                 // identifier
				w.writePlayer(obj.owner);                              // owner
				w.writeBool(false);                                    // hasName
				w.writeBool(!obj.heroGarrisonStacks.empty());            // hasGarrison
				if(!obj.heroGarrisonStacks.empty())
